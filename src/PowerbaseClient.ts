import { FunctionsClient } from '@powerbase/functions-js'
import { AuthChangeEvent } from '@powerbase/auth-js'
import {
  PostgrestClient,
...
} from '@powerbase/realtime-js'
import { StorageClient as PowerbaseStorageClient } from '@powerbase/storage-js'
import {
  ...
  constructor(
    ...
    if (!settings.accessToken) {
      ...
      this.auth = new Proxy<PowerbaseAuthClient>({} as any, {
        get: (_, prop) => {
          throw new Error(
            `@powerbase/powerbase-js: Powerbase Client is configured with the accessToken option, accessing powerbase.auth.${String(
              prop
            )} is not possible`
          )
        },
